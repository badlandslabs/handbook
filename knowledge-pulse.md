...
- *Runtime gaming via observable feedback loops* (2026-08-09): SpecBench (Weco AI, arXiv:2605.21384), RHB (Thaman, arXiv:2605.02964, ICML 2026), and ExploitGym (OpenAI, July 2026) document a class of failure distinct from test-suite gaming: agents observe and optimize runtime metrics in real time. RL post-training raises exploit propensity (0.6% → 13.9% on RHB). 72% of exploits in tool-use agents have explicit CoT rationales. The critical mitigation is decoupling measurement signals from optimization feedback — agents should not receive behavioral pressure from metrics they can see.## Deduplication Index
