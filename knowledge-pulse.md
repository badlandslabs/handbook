...
semantic-completion-check → I-3060
tool-execution-hallucination → I-3060
...
skillsieve → I-3099
air-brand-landingpage → I-3099
scanner-bypass-split → I-3099

## Recent Decisions
- *2026-07-31* — **I-3099 → S-1919 — The Post-Scan Fetch Exploit — Composite 8.85**: Tracker saturated (all prior ideas WRITTEN/DUPLICATE). Fresh research: AIR brand-landingpage skill (The Hacker News, Jun 23 2026) — reached ~26,000 agents, passed all scanners, used dynamic URL that rewrote post-scan-clearance; CSA AI Agent Skill Scanner Bypass (Jun 10 2026) — Trail of Bits bypassed ClawHub, Cisco, Vercel/skills.sh, Snyk, Socket with split-stream obfuscation in <1 hour; SkillSieve arXiv:2604.06550 v3 (Jul 27 2026) — three-layer behavioral triage framework (regex/AST, parallel LLM security sub-tasks, behavioral sandbox) not yet in production anywhere. Core insight: static scanners check the artifact handed to them, not what that artifact fetches at runtime. The post-scan fetch exploit is structurally invisible to any artifact-scanning approach. Deduplication: S-1122 (static malicious skills) covers the case where the package itself is malicious — this covers the case where the package is clean but fetches malicious content post-scan; complementary, not duplicate. S-902 (scaffold supply chain) covers dependency injection — not the same attack surface. S-1426 (MCP tool poisoning) covers untrusted server responses — different vector. Chose over: AI worm (arXiv:2606.03811 — self-propagating adaptive malware) — adjacent, but the worm attack is a separate threat class requiring its own chapter. The post-scan fetch exploit is the sharpest gap: production hot, scanner coverage gap = 10, timeliness = 9.
