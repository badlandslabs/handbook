...
## Recent Decisions
- *2026-08-11* — **I-3244 → S-2470 — The A2A Protocol Trust Stack — Composite 9.15**: Tracker re-saturated (all I-32xx ideas either WRITTEN or DUPLICATE; fresh research required). New idea from: (1) agentsid-scanner GitHub analysis on A2A JWS self-attestation gap, (2) TheCodeForge $40k timeout case from improper A2A task lifecycle management, (3) ai-threat-atlas cross-org agent card impersonation technique, (4) A2A spec task lifecycle (balalaika-tools/A2A-notes) with 7 legal states and specific failure modes. Deduplication: S-2466 (MCP trust) covers tool-server manifests (untrusted at install) — parallel but different layer (tool vs. agent handoff); S-1042 covers protocol taxonomy and MCP+A2A layers but not A2A-specific trust failures; S-1063 covers orchestration topology and coordination cost. None cover the A2A session-vs-agent auth gap or the JWKS circular trust path. Cross-links added to S-2466 (parallel trust stack), S-2465 (egress profiling prerequisite), S-1890 (escalation on delegation failure).
