...
|----------|---------|----------|-----------|
| 2026-07-05 | I-070 | WRITTEN — S-641 | Environment-Injected Memory Poisoning (eTAMP) — arXiv:2604.02623 (Zou et al., April 2026): up to 32.5% attack success on GPT-5-mini via environmental observation alone — no direct memory access needed. Cross-session, cross-site activation bypasses permission-based defenses. Gap = zero handbook coverage of eTAMP as a structural pattern (S-45 covers memory systems generically, S-289 covers red-teaming methodology, neither addresses environment-injected trajectory poisoning). Highest composite score (9.40). Highest urgency (9) + gap (10) + timeliness (10). |
