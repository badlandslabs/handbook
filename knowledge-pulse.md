...
|| I-NEW | Agent Harness Sensitivity: Why Benchmark Scores Are Scaffold Artifacts | harness-sensitivity, agent-benchmark, scaffold-artifact, swe-bench, webarena, tau-bench, tool-availability, retry-policy, judge-model, eval-gap, harness-engineering, model-selection, benchmark-provenance, scaffold-variance | 9 | 10 | 9 | 9 | 8 | **8.85** | WRITTEN — R-16 | 2026-07-25 | 2026-07-25 |
...
- *2026-07-26* — **I-3030 — Instruction Privilege Stack (S-1659) — Composite 9.20**: All 3029 prior ideas WRITTEN or DUPLICATE. Fresh research: Microsoft Spotlighting (Mar 2026), seahop/ai-threat-atlas instruction-privilege-separation, Microsoft Security Blog "Securing AI Agents" (Jun 30 2026), OWASP ASI Top 10 (Jun 2026), Unit 42 prompt injection surge (340% YoY 2026). Pattern: trust boundary migration — as agents moved from read-only to act-on-behalf, the trust boundary migrated from the network perimeter into the context window. Instruction Privilege Separation is the architectural response. Sources: Microsoft Spotlighting (Delimiting, Data Marking, Linear Attention Attribution), OWASP GenAI Security Project, Techglock (Jun 2026), Zylos Research. Deduplication: S-990 (Agent Traps/ASI01) covers attack entry points; S-1000 (Structural Governance) covers prompt guardrails; neither covers privilege hierarchy as a first-class architectural layer. Novel angle: three-layer separation (channel-level privilege tagging, instruction-level hierarchy enforcement, content-level filtering) as a unified stack. Composite 9.20 — chosen over S-1660 and S-1663 for highest urgency (prompt injection surge + OWASP #1) and deepest coverage gap.
...
- *2026-07-26* — **Observability as Substrate**: Observability (traces capture what happened) and evaluation (was it correct) are distinct but complementary. Conflating them produces neither — traces are prerequisite for both AI SRE detection (S-1005) and behavioral regression eval (S-1005/S-3022). The practical stack: OpenTelemetry GenAI semconv (5 agent span operations, v1.41.1, now in dedicated repo semantic-conventions-genai) + Langfuse/LangSmith/AgentOps as backend + cross-agent conversation IDs for span correlation. GenAI conventions remain experimental (no committed 1.0 date as of Jul 2026); use framework instrumentation libraries (Traceloop, LangChain OTel integration) to absorb attribute schema churn. LangSmith has ~0% overhead vs 5-40% for other platforms on multi-agent workloads (AIMultiple, Jul 2026).
