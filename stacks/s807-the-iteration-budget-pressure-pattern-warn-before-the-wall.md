# S-807 · The Iteration Budget Pressure Pattern: Warn Before the Wall
...
- **GitHub Issue:** Iteration Budget Pressure — Warn the LLM Before Max Iterations Hit — Hermes Agent issue #414, opened March 5, 2026, with a proposed two-tier implementation (caution at 70%, warning at 90%) and a PR (#54277) tracking the fix. Explicitly cites Utah (Inngest's agent harness) as the inspiration for the pattern. — [github.com/NousResearch/hermes-agent/issues/414](https://github.com/NousResearch/hermes-agent/issues/414)
- **GitHub Repo:** Utah — Universally Triggered Agent Harness — an Inngest-powered durable agent that implements the think/act/observe loop with budget pressure as a core design principle. Every LLM call is an Inngest step with retry and cancellation support. — [github.com/inngest/utah](https://github.com/inngest/utah)
- **Case Study:** A documented incident from November 2025: a multi-agent LangChain system ran undetected for 11 days with two agents locked in a recursive loop, accumulating $47,000 in API charges before discovery via monthly billing. Root causes: no per-agent spend limit, no runtime timeout, no anomaly detection on iteration count. — [kognita.co/blog/ai-agent-runaway-cost-no-kill-switch](https://www.kognita.co/blog/ai-agent-runaway-cost-no-kill-switch)
- **Post-mortem:** The same incident reported on DEV Community — "The AI Agent That Cost $47,000 While Everyone Thought It Was Working." Details the architectural failure: no orchestrator watching the full conversation, no shared memory between agents, no global state. — [dev.to/utibe_okodi_339fb47a13ef5](https://dev.to/utibe_okodi_339fb47a13ef5/the-ai-agent-that-cost-47000-while-everyone-thought-it-was-working-1lg6)
- **Pattern Analysis:** TeachYou Academy — "Agent Guardrails: Preventing Runaway Loops and Cost Overruns," July 2026. Names the five failure modes driving runaway costs: tool loops, context explosion, no max_tokens, model confusion loops, and plan deviation. Frames guardrails as "explicit, testable limits enforced in code rather than hoped for in a prompt." — [teachyou.ai/blog/agent-guardrails-runaway-loops](https://www.teachyou.ai/blog/agent-guardrails-runaway-loops)
...
- **Noisy at scale** — injecting messages every API call (checking thresholds) adds latency on the order of microseconds. Profile before claiming it's negligible in high-throughput pipelines.
